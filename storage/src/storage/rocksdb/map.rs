...
#[derive(Clone)]
pub struct DataMap<K: Serialize + DeserializeOwned, V: Serialize + DeserializeOwned, T: StorageWritability> {
    pub(super) storage: RocksDB<T>,
    pub(super) context: Vec<u8>,
    pub(super) _phantom: PhantomData<(K, V)>,
}

impl<K: Serialize + DeserializeOwned, V: Serialize + DeserializeOwned, T: StorageWritability> DataMap<K, V, T> {
    #[inline]
...
impl<K: Serialize + DeserializeOwned, V: Serialize + DeserializeOwned, T: StorageWritability> fmt::Debug for DataMap<K, V, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DataMap").field("context", &self.context).finish()
    }
}

impl<'a, K: Serialize + DeserializeOwned, V: Serialize + DeserializeOwned, T: StorageWritability> MapReadOnly<'a, K, V>
    for DataMap<K, V, T>
{
...
impl<'a, K: Serialize + DeserializeOwned, V: Serialize + DeserializeOwned, T: StorageReadWrite> MapReadWrite<'a, K, V>
    for DataMap<K, V, T>
{
    ///
