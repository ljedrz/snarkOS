...
#[derive(Debug)]
pub struct LedgerState<N: Network, T: StorageWritability> {
...
impl<N: Network, T: StorageReadOnly> LedgerState<N, T> {
    ...
    ///
    pub fn open_reader<S: Storage<Writability = T>, P: AsRef<Path>>(path: P) -> Result<(Arc<Self>, Resource)> {
        // Open storage.
        ...
        let ledger = Arc::new(Self {
            ...
            blocks: BlockState::open_ro(storage)?,
            read_only: (is_read_only, RwLock::new(N::genesis_block().clone())),
        });

        let _ = ledger.ledger_roots.refresh();
...
impl<N: Network, T: StorageReadWrite> LedgerState<N, T> {
    ///
    ...
    /// a read-only instance of `LedgerState` may only call immutable methods.
    ///
    pub fn open_writer<S: Storage<Writability = T>, P: AsRef<Path>>(path: P) -> Result<Self> {
    ...
    #[doc(hidden)]
    pub fn open_writer_with_increment<S: Storage<Writability = T>, P: AsRef<Path>>(path: P, validation_increment: u32) -> Result<Self> {
        // Open storage.
        let context = N::NETWORK_ID;
        ...
        let ledger = Self {
            ...
            latest_block_locators: Default::default(),
            ledger_roots: storage.open_map(MapId::LedgerRoots)?,
            blocks: BlockState::open_rw(storage)?,
            read_only: (is_read_only, RwLock::new(N::genesis_block().clone())),
        };
...
#[derive(Clone, Debug)]
struct BlockState<N: Network, T: StorageWritability> {
    block_heights: DataMap<u32, N::BlockHash, T>,
    block_headers: DataMap<N::BlockHash, BlockHeader<N>, T>,
    block_transactions: DataMap<N::BlockHash, Vec<N::TransactionID>, T>,
    transactions: TransactionState<N, T>,
}

impl<N: Network, T: StorageReadOnly> BlockState<N, T> {
    ...
    fn open_ro<S: Storage<Writability = T>>(storage: S) -> Result<Self> {
        Ok(Self {
            ...
            transactions: TransactionState::open_ro(storage)?,
        })
    }
...
impl<N: Network, T: StorageReadWrite> BlockState<N, T> {
    /// Initializes a new instance of `BlockState`.
    fn open_rw<S: Storage<Writability = T>>(storage: S) -> Result<Self> {
        Ok(Self {
            ...
            block_headers: storage.open_map(MapId::BlockHeaders)?,
            block_transactions: storage.open_map(MapId::BlockTransactions)?,
            transactions: TransactionState::open_rw(storage)?,
        })
    }
...
struct TransactionState<N: Network, T: StorageWritability> {
    transactions: DataMap<N::TransactionID, (N::LedgerRoot, Vec<N::TransitionID>, Metadata<N>), T>,
    transitions: DataMap<N::TransitionID, (N::TransactionID, u8, Transition<N>), T>,
    serial_numbers: DataMap<N::SerialNumber, N::TransitionID, T>,
    commitments: DataMap<N::Commitment, N::TransitionID, T>,
}

impl<N: Network, T: StorageReadOnly> TransactionState<N, T> {
...
impl<N: Network, T: StorageReadWrite> TransactionState<N, T> {
    /// Initializes a new instance of `TransactionState`.
    fn open_rw<S: Storage<Writability = T>>(storage: S) -> Result<Self> {
        Ok(Self {
            transactions: storage.open_map(MapId::Transactions)?,
